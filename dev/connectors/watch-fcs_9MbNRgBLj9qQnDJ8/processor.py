...
def get_param_mapping(
    ...
    id = id_group.group()
    return {
        "Ingest_FCS.file_pattern": fp(watch_dir, parent_dir, f"*.fcs"),
    }
...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> TriggerFlowParams:
    ...
    for param, files in flow_params_fw.files.items():
        (
            ...
            experiment_step,
            well,
        ) = files.filename.split(".")[0].split("_")
        files = add_file_tag(file_param=files, tag_type_id="run_type", display_value=run_type)
        files = add_file_tag(
            file_param=files, tag_type_id="experiment_day", display_value=day_number
        )
        files = add_file_tag(
            file_param=files, tag_type_id="experiment_step", display_value=experiment_step
        )
