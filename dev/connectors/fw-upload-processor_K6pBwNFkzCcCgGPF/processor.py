...
from agent_sdk import FileWatcherResult, TagFilesOnlyParams, info
from gm_api import UnappliedFileTag
...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> TagFilesOnlyParams | None:
    info("RUNNING PROCESSOR - TAGGING FILES")
    file_params = list(flow_params_fw.files.values())
    for fp in file_params:
        fp.tags.append(UnappliedFileTag(
            tag_type_id="data_lifecycle_stage",
            display_value="fw-upload",
        ))
