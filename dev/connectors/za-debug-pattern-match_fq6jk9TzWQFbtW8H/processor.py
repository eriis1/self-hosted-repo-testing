...
def fp(watch_dir: str, parent_dir: str, pattern: str | List[str]) -> Callable[[str], bool]:
    """
    ...
    else:  # pattern is a list

        def fp_res(x: str):
            x = parse.unquote(x)
            flat_list = []
            for p in pattern:
                flat_list += glob.glob(os.path.join(watch_dir, p), recursive=True)
            return x in flat_list
...
def get_param_mapping(
    ...
    return {
        "Excel_Read.excel": fp(watch_dir, parent_dir, [r"*.xls", r"*.xlsx"]),
    }
