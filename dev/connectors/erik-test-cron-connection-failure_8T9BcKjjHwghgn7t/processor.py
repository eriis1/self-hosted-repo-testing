...
def execute(**kwargs) -> TriggerFlowParams | None:
    ...
    if system_log_trigger or run_log_trigger:  # TODO separate out if necessary
        ...
        # check if well log already fully processed

        splitPath = well_log_filepath.split("\\")
        filename_noext, fileext = os.path.splitext(splitPath[-1])

        already_processed = read_sql_query(
            f"SELECT Well_Log_File_Name FROM ABC_File_Validation_Results WHERE Well_Log_File_Name like '{filename_noext}%'"
        )

        debug(f"already_processed {already_processed}")
        if (
            already_processed is not None
            and "Well_Log_File_Name" in already_processed.columns
            and len(already_processed) > 0
        ):
            debug(f"already fully processed file {well_log_filepath}, skipping")
            return None
        ...
        return UploadFileParams(
            files=[system_log_file_param, run_log_file_param, well_log_file_param]
        )
