...
from agent_sdk import FileParam, FileWatcherResult, TriggerFlowParams, debug
...
def get_param_mapping(
    ...
    return {
        "Input_File.file_pattern": fp(watch_dir, parent_dir, f"*.pdf"),
...
def execute(flow_params_fw: FileWatcherResult, **kwargs) -> TriggerFlowParams | None:
    ...
    single_file_param_key = "Input_File.file_pattern"
    csv_file_param_key = "CSV_Read.csv"
    multi_file_param_key = "Input_File.file_pattern"

    params = TriggerFlowParams()

    for param in flow_params_fw.files.values():
        if isinstance(param, FileParam):
            if FileParam.filename.endswith("csv"):
                params.add_file_param(param_node_key=csv_file_param_key, file=param)
            else:
