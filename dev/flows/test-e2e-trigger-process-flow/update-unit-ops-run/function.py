...
def execute(
    ...
    # get instrument run 
    get_unit_ops_run_url = f"https://{host}/v1/environment/{environment}/unit-operation-runs/external/{g.flow_run_id}"
    response = requests.get(get_unit_ops_run_url, headers=headers)
    data = json.loads(response.text)
    unit_ops_run_id = data[0]['id']  
    
    # update instrument run
    unit_ops_runs_url = f"https://{host}/v1/environment/{environment}/unit-operation-runs/{unit_ops_run_id}"
    body = {
    ...
    try:
        response = requests.patch(unit_ops_runs_url, headers=headers, data=json.dumps(body))
