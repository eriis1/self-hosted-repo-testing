...
def execute(
    ...
    # create unit ops run
    process_runs_url = f"https://{host}/v1/environment/{environment}/process-runs"
    body = {
        "processId": 1,
        "flowRuns": [
            {
        ...
        "fileUris": [uri],
        "url": "http://example.com/process-run",
        "externalId": flow_run_id,
        "bqTableNames": ["process_run_table"],
    ...
    try:
        response = requests.post(process_runs_url, headers=headers, data=json.dumps(body))
