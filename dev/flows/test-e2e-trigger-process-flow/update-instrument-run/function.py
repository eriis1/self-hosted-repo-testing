...
def execute(
    ...
    # get instrument run 
    get_instrument_run_url = f"https://{host}/v1/environment/{environment}/instrument-runs/external/{g.flow_run_id}"
    response = requests.get(get_instrument_run_url, headers=headers)
    data = json.loads(response.text)
    instrument_run_id = data[0]['id']   
    tables = g.list_tables_current_run()
    print("All tables:")
    print(tables)
    first_table_name = tables["table_name"].iloc[0]

    # update instrument run
    instrument_runs_url = f"https://{host}/v1/environment/{environment}/instrument-runs/{instrument_run_id}"
    body = {
        "endTime": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "state": "Completed",
    }
    try:
        response = requests.patch(instrument_runs_url, headers=headers, data=json.dumps(body))
