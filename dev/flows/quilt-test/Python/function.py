...
def execute(
    ...
    requirements_example = p["requirements.txt"].deserialize().strip().split()
    df = pd.DataFrame({"requirements": requirements_example})

    return NodeReturn(tables_to_upload={"quilt_requirements": df})